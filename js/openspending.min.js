 ...
 * limitations under the License.
 */
;var OpenSpending=OpenSpending||{};OpenSpending.CSVloader=function(){this.get=function(a){CSV.fetch({url:a.aggregated_csv_url}).done(function(i){var h=i.fields.slice(0,i.fields.length-1),k=a.amount_col_name||"amount",f=i.fields.indexOf(k),d=a.currency;function j(l){if(l!==null){return l.toString().toLowerCase().replace(/\s+/g,"-").replace(/[^\w\-]+/g,"").replace(/\-\-+/g,"-").replace(/^-+/,"").replace(/-+$/,"")}else{return"null_id"}}function e(q,p,n,o,l){this.id=String(p);this.name=String(n);this.amount=o;this.label=String(n);this.level=q;this.children=[];this.currency=l;this.addchild=function(u){for(var t in this.children){var v=this.children[t];if(v.id===u.id){v.amount+=u.amount;return v}}this.children.push(u);return u}}var c=new e(0,"root","root",0,d);i.records.forEach(function(n){var l=function(q,o,p){if(p===(h.length)){}else{l(q.addchild(new e(p+1,j(n[p]),n[p],Number(n[f]),d)),h,p+1)}if(p===0){c.amount+=Number(n[f])}};l(c,h,0)});a.callback(c)});return{}}};
